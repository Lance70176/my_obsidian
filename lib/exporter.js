...
function loadHighlightCss() {
  ...
  const dark = fs.readFileSync(path.join(stylesDir, 'github-dark.min.css'), 'utf8');
  return `${light}\n@media (prefers-color-scheme: dark) {\n${dark}\n}`;
}
...
function exportNote(mdPath, outPath) {
  ...
  fs.writeFileSync(outPath, htmlDocument(noteTitle(mdPath), body));
  return outPath;
}
...
module.exports = { exportNote, exportVault, htmlDocument, walkVault, buildNoteIndex, mdHrefToHtml };
